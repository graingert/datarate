...
class SampleThingLoading(TestCase):
	def setUp(self):
		...
		schema_file = os.path.dirname(reviews.__file__) + "/static/things/thing.schema.json"
	...
	def test_good(self):
		...
		resp = self.client.get("/thing.json?{data}".format(data=urllib.urlencode({"uri":"http://id.southampton.ac.uk/point-of-service/65-avenue"})), follow=True)
		self.assertEqual(resp.status_code, 200)
		validate(json.loads(resp.content), self.schema)
	
	def test_malformed(self):
		resp = self.test_uri("http://id.southampton.ac.uk/point-of-service/65-avenu")
